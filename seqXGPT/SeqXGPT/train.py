...
class SupervisedTrainer:
    ...
    def sent_level_eval(self, texts, true_labels, pred_labels):
        ...
        full_sents = []
        for text, true_label, pred_label in zip(texts, true_labels, pred_labels):
            sents, true_sent_label = self.get_sent_label(text, true_label)
            sents, pred_sent_label = self.get_sent_label(text, pred_label)
            full_sents.extend(sents)
        ...
        write_fp = open('small_sent_level_eval.txt', 'w')
        ...
        print(cohen_kappa_score(true_sent_labels, pred_sent_labels))
...
def split_dataset(train_input_path, test_input_path, train_path, test_path, train_ratio=0.9):
    print('*'*32)
...
def parse_args():
    ...
    parser.add_argument('--train_input_path', type=str, default='')
    parser.add_argument('--test_input_path', type=str, default='')
...
if __name__ == "__main__":
    ...
    if args.split_dataset:
        ...
        split_dataset(train_input_path=args.train_input_path,test_input_path=args.test_input_path, train_path=args.train_path, test_path=args.test_path, train_ratio=args.train_ratio)
